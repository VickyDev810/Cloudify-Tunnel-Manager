...
class UniversalTunnelManager:
    ...
    def _kill_process_by_pattern(self, pattern: str):
        """Kill processes matching a pattern (cross-platform)"""
        if sys.platform == 'win32':
            # Windows: Use taskkill with wildcard matching
            try:
    ...
    def _check_process_by_pattern(self, pattern: str) -> bool:
        """Check if a process matching pattern exists (cross-platform)"""
        if sys.platform == 'win32':
            # Windows: Use wmic to check for process
            try:
                result = subprocess.run(
                    ['wmic', 'process', 'where', f'CommandLine like "%{pattern}%"', 'get', 'ProcessId'],
                    capture_output=True,
                ...
                # Fallback: check if cloudflared.exe is running
                try:
    ...
    def _check_tunnel_status(self, tunnel_name: str) -> str:
        ...
        if self._check_process_by_pattern(f"cloudflared.*tunnel.*{tunnel_name}"):
        ...
        if tunnel_info.get("temp_process_id"):
            try:
                if sys.platform == 'win32':
                    ...
                    result = subprocess.run(['tasklist', '/FI', f'PID eq {tunnel_info["temp_process_id"]}'],
                                          capture_output=True, text=True)
                    if str(tunnel_info["temp_process_id"]) in result.stdout:
                        return "running (temp)"
                else:
    ...
    def _restart_tunnel(self):
        ...
        if self.check_autostart_status(verbose=False):
            ...
            if sys.platform == 'win32':
                # Windows: restart scheduled task
                ...
                try:
                    ...
                    print("Windows service restarted")
                    return
                except:
                    print("Failed to restart Windows service, falling back to manual restart")
            elif self._has_systemd_user_support():
                try:
        ...
        # Manual restart
        self._kill_process_by_pattern(f"cloudflared.*tunnel.*{self.tunnel_name}")
        time.sleep(2)
        ...
        if sys.platform == 'win32':
            ...
            subprocess.Popen(
                f'start /B cloudflared tunnel --config "{self.config_file}" run {self.tunnel_name}',
                shell=True,
...
<dict>
    ...
    def delete_tunnel(self, tunnel_name: str = None, force: bool = False) -> bool:
        ...
        # Kill processes
        self._kill_process_by_pattern(f"cloudflared.*tunnel.*{tunnel_name}")
            ...
            try:
                ...
                if "active connections" in str(e.stderr):
                    ...
                    if retry_count < max_retries:
                        ...
                        if sys.platform == 'win32':
                            print(f"  taskkill /F /IM cloudflared.exe")
                        else:
    ...
    def start_tunnel(self, tunnel_name: str = None):
        ...
        if sys.platform == 'win32':
            # Windows: Use START command to run in background
            subprocess.Popen(
                f'start /B cloudflared tunnel --config "{config_file}" run {tunnel_name}',
                shell=True
            )
        else:
            subprocess.Popen([
