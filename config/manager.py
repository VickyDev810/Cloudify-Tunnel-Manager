...
class UniversalTunnelManager:
    ...
    def create_tunnel(self, tunnel_name: str = None, setup_auto_start: bool = None) -> bool:
        ...
        if tunnel_exists:
            ...
            if setup_auto_start and not self.check_autostart_status(verbose=False):
                print("\nAuto-start is not configured for this tunnel.")
                setup = input("Would you like to set up auto-start? (y/n): ").lower()
                if setup == 'y':
        ...
        try:
            ...
            if setup_auto_start:
                ...
                setup = input("Enable tunnel to start automatically? (y/n): ").lower()
                if setup == 'y':
                    self.setup_autostart()
    ...
    def add_route(self, domain: str, localhost_port: int, end_user_url: str = "localhost", tunnel_name: str = 'Universal-Tunnel') -> bool:
        ...
        if is_first_route and self.auto_start_on_create:
            if not self.check_autostart_status(verbose=False):
                ...
                setup = input("Would you like the tunnel to start automatically? (y/n): ").lower()
                if setup == 'y':
                    self.setup_autostart()
...
<dict>
    ...
    def delete_tunnel(self, tunnel_name: str = None, force: bool = False) -> bool:
        ...
        if not force:
            confirm = input(f"Delete tunnel '{tunnel_name}' and all routes? (yes/no): ")
